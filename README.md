# 🍲 Best Recipe

**Best Recipe** - an educational project, used to demonstrate basic capabilities of a recipe app

User can view trending and popular recipes fetched from Spoonacular API, view recipe details, save recipes, create recipes & customise their profile.

---

## ⚙️ If you would like to use this project in any way

Use for educational purposes.

Credit and tag authors if used.

Follow github usage guidelines.

---

Special thanks to:

Irina (Github: https://github.com/irinazakhvatkina)

Zarina (Github: https://github.com/ZarinaSadykovaQA)

© 2025 Best Recipe. All rights reserved.
